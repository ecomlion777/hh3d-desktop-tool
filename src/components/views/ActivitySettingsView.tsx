    ...
    <div id="view-activity-settings" className="p-4 space-y-4 overflow-y-auto custom-scrollbar h-full text-slate-200">
      <div className="bg-slate-950 p-4 border border-slate-800 rounded-lg flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="font-bold text-sm text-slate-100">Phase 07 — Module Framework</h2>
            <p className="text-xs text-slate-400">Registry, cấu hình theo profile, chạy module có kiểm soát và runtime status</p>
          </div>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 space-y-3">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-800 pb-3">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-1">Danh mục được xây từ các chức năng của userscript v5.4.8. Phase 07 chưa chuyển hành động game.</p>
          </div>
          <select
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 space-y-4">
          ...
          <div className="space-y-2 text-[11px] text-slate-400">
            ...
            <div className="flex items-center gap-2"><Construction className="w-3.5 h-3.5 text-amber-400" /> Module `planned` chỉ lưu cấu hình, không gọi API</div>
          </div>
